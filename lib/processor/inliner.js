...
function inline(filePath, options) {
    ...
    if (_.isString(filePath)) {
        var inlineResult = Inliner.getCache(filePath);
        if (inlineResult) {
            return inlineResult;
        }
    ...
    inlineResult = file;
    debug('inline file: %s', file.path);

    !file.owner && (Inliner.cache(filePath, inlineResult));
 ...
 * 缓存内联的结果
 *
 * @param {string} path 内联的文件路径
 * @param {Object} result 内联的结果
 */
Inliner.cache = function (path, result) {
    inlineCache[path] = result;
};

/**
 * 获取内联缓存的结果
 *
 * @param {string} path 内联的路径
 * @return {?Object}
 */
Inliner.getCache = function (path) {
    return inlineCache[path];
};
...
Inliner.inline = function (options) {
    ...
    processFiles.forEach(function (file) {
        var processor = getInlineProcessor(file, options);
        if (processor) {
            var result = processor.inline();
            Inliner.cache(file.path, result);
        }
    });
